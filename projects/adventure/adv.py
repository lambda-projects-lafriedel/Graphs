...
# Fill this out
'''
FUNCTION TO GRAB ALL EXITS AND CREATE GRAPH ENTRY
From current room, grab all of its exits
Generate a dict that contains all of its exits as keys and instantiate their values with '?'
Add that current room, that dict as k, v to graph

DFT UNTIL DEAD END
From current room, pick a random direction and do a depth first traversal
When direction is chosen, travel to new room in chosen direction
Update dict at key of current room, where the direction key is updated to the value of the new room
Do the opposite. So should I manually store opposites of directions in some way? A dict? Tuples? Doing the opposite is how the DFT will get to the end
Traversal ends when it ends up in a room that does not contain any unexplored exits

BFT: SHORTEST PATH TO NEXT ROOM WITH UNEXPLORED EXITS
From dead end room, do a BFT to find the closest room possible to walk to that has unexplored exits
Travel to room in the one direction available
Check if that room has any unexplored exits
If not, continue the loop on the new room
Continue loop until a room is found to have in its dict any keys that have a value of '?'
'''
graph = dict()
traversalPath = list()
path_away = set()
...
def dfs_until_dead_end(room):
    ...
    while len(s) > 0:
        ...
        # print("ROOM ID IN DFS", curr_roomID)
        current = s.pop()
        ...
        # print("DIR IN DFS", direction, curr_roomID)
        ...
        if direction is not None:
            ...
            curr_roomID = player.currentRoom.id
            curr_room = player.currentRoom
            # update graph key with the id of new room
            graph[prev_roomID][direction] = curr_roomID
            # if new room is not currently in graph
...
def bfs_until_room_with_unexplored_exit(room):
    q = [] # [[121,148]]
    q.append([room.id])
    curr_room = room # 121 / 148
    # while queue length is > 0
    while len(q) > 0 and len(graph) is not 500:
        unexplored = get_unexplored_direction(curr_room)
        if unexplored is None: # 121: true / 
            # dequeue the id
            curr_path = q.pop(0) 
            last_visited = curr_path[-1] # 121 /
            #print("LAST VISITED", last_visited)
            #print("PATH AWAY", path_away)
            #print(f'{graph}\n')
            if last_visited not in path_away: # 121: true
                for direction, roomID in graph[last_visited].items(): # [n, s, e]
                    path_away.add(last_visited)
                    # print("ROOM ID IN FOR", roomID)
                    # print("ITEMS", last_visited, graph[last_visited].items())
                    if len(visited_rooms) is len(roomGraph):
                        return curr_room
                    elif roomID in path_away:
                        continue
                    elif roomID == '?':
                        player.travel(direction)
                        traversalPath.append(direction)
                        curr_room = player.currentRoom
                        return curr_room
                    elif roomID not in path_away:
                        #print("ROOM ID", roomID)
                        # print("DIR IN BFS", direction, curr_room.id)
                        player.travel(direction) # from 121 to 148 / 
                        traversalPath.append(direction)
                        curr_room = player.currentRoom
                        add_dest = list(curr_path)
                        add_dest.append(roomID)
                        q.append(add_dest)
                    else:
                        continue
                return curr_room
            else:
                # i have to return something here and make it so that the search can continue
                # but if i just return curr_room, it goes in an endless loop between dfs and bfs
                # grab the exits with getExits
...
def do_the_thing(starting_room):
    room = starting_room
    # while len(visited_rooms) < len(room_graph):
    while len(graph) is not len(roomGraph):
        dead_end = dfs_until_dead_end(room)
        #dfs until dead end reached, dead end is returned
        # set variable of dfs as param given to bfs
        continue_search = bfs_until_room_with_unexplored_exit(dead_end)
        #bfs runs until unexplored room is found
        # set variable set as bfs to room
        room = continue_search
...
do_the_thing(player.currentRoom)
...
def dft_test(starting_room):
    ...
    curr_room_id = starting_room # 0 / 1
    s.append(curr_room_id)
    graph[curr_room_id] = {"n": "?", "s": "?", "w": "?", "e": "?"}
    ...
    while len(s) > 0:
        ...
        if room not in visited:
            if room not in graph:
                graph[curr_room_id] = {"n": "?", "s": "?", "w": "?", "e": "?"}
            visited.append(room)
            # for each direction that has an exit from curr room
            for direction in player.currentRoom.getExits(): # n, s
                print("DIR", direction)
                # if room in that direction not in visited path
                if 
                    # add direction to traversalPath
                    traversalPath.append(direction)
                    # make player travel in that direction
                    player.travel(direction)
                    # get the id of the new room
                    new_room_id = player.currentRoom.id # 1
                    print(curr_room_id, new_room_id)
                    # in graph, add room id to direction value of curr room id key
                    print(graph)
                    graph[curr_room_id][direction] = new_room_id
                    # add the id of the new room to the stack
                    s.append(new_room_id)
                    print("STACK APPEND", s)
                    # change current room id to new room
